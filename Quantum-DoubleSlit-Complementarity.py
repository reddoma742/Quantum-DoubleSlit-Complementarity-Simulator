...
lam = de_broglie_wavelength(v_par)
sigma = a_width * L / lam
I_left = np.exp(-(x + d_slit/2)2 / (2 * sigma2))
I_right = np.exp(-(x - d_slit/2)2 / (2 * sigma2))
...
# Main pattern
ax1.plot(x1000, I, 'b-', lw=1.5)
ax1.fill_between(x1000, I, alpha=0.3)
...
ax3.plot(x1000, I_interf_norm, 'b--', lw=1, alpha=0.7, label='Pure interference')
ax3.plot(x1000, I_particle_norm, 'r--', lw=1, alpha=0.7, label='Pure which‑path')
...
# Info panel with complementarity value
info = (f"Fringe spacing: {spacing_mm:.2f} mm\n"
f"λ = {lam1e9:.2f} nm\n"
f"v = {v_mean/1e3:.0f} km/s\n"
f"Slit width = {a_width1e6:.2f} µm\n"
